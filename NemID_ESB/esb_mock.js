...
app.get("/test", (req, res) => {
  res
    .status(200)
    .send({ message: "Server is running just fine on port 8080... " });
});
...
app.post(
  ...
  async (req, res, next) => {
    ...
    let nemId = "";
    console.log(cpr, email, nemId);
    axios
      .post("http://localhost:8088/generate-nemId", { cpr: cpr, email: email })
      .then((response) => {
        nemId = response.data.nemId;
        axios
          .post("http://localhost:8089/generate-password-nemID", {
            nemId: nemId,
            cpr: cpr,
          })
          .then((re) => {
            console.log(
              "Insterting following: ",
              cpr,
              nemId,
              re.data.nemIdPassword
            );
...
app.post("/generate-auth-code", async (req, res) => {
  let data = req.body;
  axios
    .post("http://localhost:8090/nemid-auth", data)
    .then((response) => {
      console.log(response);
      return res
        .status(200)
        .send({ generatedCode: response.data.generatedCode });
    })
    .catch((err) => {
      if (err) {
        console.log(err);
